package net.geidea.payment.kernelconfig.viewmodel

import android.content.Context
import android.os.Bundle
import android.util.Log
import androidx.lifecycle.LiveData
import androidx.lifecycle.MutableLiveData
import androidx.lifecycle.ViewModel
import com.pos.sdk.emvcore.POIEmvCoreManager
import com.pos.sdk.emvcore.POIEmvCoreManager.EmvTerminalConstraints
import com.pos.sdk.emvcore.PosEmvAid
import com.pos.sdk.emvcore.PosEmvCapk
import com.pos.sdk.utils.PosUtils
import dagger.hilt.android.lifecycle.HiltViewModel
import dagger.hilt.android.qualifiers.ApplicationContext
import net.geidea.payment.kernelconfig.model.AmexParameter
import net.geidea.payment.kernelconfig.model.DiscoverParameter
import net.geidea.payment.kernelconfig.model.EntryPoint
import net.geidea.payment.kernelconfig.model.MirParameter
import net.geidea.payment.kernelconfig.model.QvsdcParameter
import net.geidea.payment.tlv.BerTag
import net.geidea.payment.tlv.BerTlv
import net.geidea.payment.tlv.BerTlvBuilder
import net.geidea.payment.tlv.HexUtil
import net.geidea.payment.utils.IS_KERNEL_CONFIG_COMPLETED
import net.geidea.utils.DataStoreUtils
import net.geidea.utils.extension.ioCoroutine
import javax.inject.Inject


@HiltViewModel
class KernelConfigViewModel @Inject constructor(@ApplicationContext val context: Context) :
    ViewModel() {

    private val tag = "KernelConfigViewModel"
    private var isKernelConfigCompleted = MutableLiveData<Boolean>()


    init {
        ioCoroutine {
            loadTerminal()
            loadAid()
            loadCapk()
            loadVisa()
            loadUnionPay()
            loadMasterCard()
            loadDiscover()
            loadAmex()
            loadMir()
            DataStoreUtils.getInstance(context).saveBoolean(IS_KERNEL_CONFIG_COMPLETED, true)
            isKernelConfigCompleted.postValue(true)
        }

    }

    private fun loadTerminal() {
        val emvCoreManager = POIEmvCoreManager.getDefault()

        val bundle = Bundle().apply {
            // EMV TAG 9F1C Len 8
            putByteArray(EmvTerminalConstraints.TERMINAL_ID, "1234567890".toByteArray())

            // EMV TAG 9F16 Len 15
            putByteArray(EmvTerminalConstraints.MERCHANT_ID, "000000000000000".toByteArray())
            // EMV TAG 9F15 Len 4
            putByteArray(EmvTerminalConstraints.MERCHANT_CATEGORY_CODE, "0840".toByteArray())

            // EMV TAG 9F4E Len V
            putByteArray(EmvTerminalConstraints.MERCHANT_NAME, "Geidea Fintech".toByteArray())

            // EMV TAG 9F1A Len 4
            putByteArray(EmvTerminalConstraints.TERMINAL_COUNTRY_CODE, "0966".toByteArray())

            // EMV TAG 5F2A Len 4
            putByteArray(EmvTerminalConstraints.TRANS_CURRENCY_CODE, "0682".toByteArray())

            // EMV TAG 5F36 Len 2
            putByteArray(EmvTerminalConstraints.TRANS_CURRENCY_EXP, "02".toByteArray())

            // EMV TAG 9F3C Len 4
            putByteArray(EmvTerminalConstraints.TRANS_REFER_CURRENCY_CODE, "0682".toByteArray())

            // EMV TAG 9F3D Len 2
            putByteArray(EmvTerminalConstraints.TRANS_REFER_CURRENCY_EXP, "02".toByteArray())

            // EMV TAG 9F35 Len 2

            // EMV TAG 9F35 Len 2
            putByteArray(EmvTerminalConstraints.TERMINAL_TYPE, "22".toByteArray())

            // EMV TAG 9F33 Len 6
            putByteArray(EmvTerminalConstraints.TERMINAL_CAPABILITY, "E0F8C8".toByteArray())

            // EMV TAG 9F40 Len 10
            putByteArray(
                EmvTerminalConstraints.TERMINAL_EX_CAPABILITY, "F000F0F001".toByteArray()
            )
            // EMV TAG 9F1E Len 8
//        bundle.putByteArray(EmvTerminalConstraints.IFD_SERIAL_NUMBER,
//                POIGeneralAPI.getDefault().getVersion(POIGeneralAPI.VERSION_TYPE_DSN).getBytes());
            // EMV TAG 9F39 Len 2
            // EMV TAG 9F1E Len 8
//        bundle.putByteArray(EmvTerminalConstraints.IFD_SERIAL_NUMBER,
//                POIGeneralAPI.getDefault().getVersion(POIGeneralAPI.VERSION_TYPE_DSN).getBytes());
            // EMV TAG 9F39 Len 2
            putByteArray(EmvTerminalConstraints.TERMINAL_ENTRY_MODE, "05".toByteArray())



            putBoolean(EmvTerminalConstraints.PSE, true)
            putBoolean(EmvTerminalConstraints.CARD_HOLDER_CONFIRM, true)
            putBoolean(EmvTerminalConstraints.LANGUAGE_SELECT, true)
            putBoolean(EmvTerminalConstraints.DEFAULT_DDOL, true)
            putBoolean(EmvTerminalConstraints.DEFAULT_TDOL, true)


            putBoolean(EmvTerminalConstraints.BYPASS_PIN_ENTRY, true)
            putBoolean(EmvTerminalConstraints.SUBSEQUENT_BYPASS_PIN_ENTRY, true)


            putBoolean(EmvTerminalConstraints.GET_DATA_FOR_PIN_COUNTER, true)



            putBoolean(EmvTerminalConstraints.FLOOR_LIMIT_CHECKING, true)
            putBoolean(EmvTerminalConstraints.RANDOM_TRANSACTION_SELECTION, true)


            putBoolean(EmvTerminalConstraints.VELOCITY_CHECKING, true)
            putBoolean(EmvTerminalConstraints.EXCEPTION_FILE, true)
            putBoolean(EmvTerminalConstraints.REVOCATION_ISSUER_PUBLIC_KEY, true)
            // bundle.putBoolean(EmvTerminalConstraints.ISSUER_REFERRAL, false);
            // bundle.putBoolean(EmvTerminalConstraints.UNABLE_TO_GO_ONLINE, false);  //那个是联机拒绝后, 走IAC_DEFAULT 判断.


            // Mandatory online, mandatory approval.
            // bundle.putBoolean(EmvTerminalConstraints.ISSUER_REFERRAL, false);
            // bundle.putBoolean(EmvTerminalConstraints.UNABLE_TO_GO_ONLINE, false);  //那个是联机拒绝后, 走IAC_DEFAULT 判断.

            // Mandatory online, mandatory approval.
            putBoolean(EmvTerminalConstraints.FORCED_ONLINE, false)
            putBoolean(EmvTerminalConstraints.FORCED_ACCEPT, false)
        }

        emvCoreManager.EmvSetTerminal(EmvTerminalConstraints.TYPE_TERMINAL, bundle)

    }

    private fun loadAid() {
        val emvCoreManager = POIEmvCoreManager.getDefault()
        // Delete all configured aid
        emvCoreManager.EmvDeleteAid()

        // VISA
        var aid = addAid("A0000000031010", "008C")
        aid.FloorLimit = 0
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )

        aid.TypeIndicator = false
        emvCoreManager.EmvSetAid(aid)

        aid = addAid("A0000000032010", "008C")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)

        aid = addAid("A0000000033010", "008C")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)


        // Unionpay
        aid = addAid("A000000333010101", "0020")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0010000000")
        aid.TACOnline = PosUtils.hexStringToBytes("d84004f800")
        aid.TACDefault = PosUtils.hexStringToBytes("084000a800")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, HexUtil.parseHex("9F66046C004000"), null, null
        )
        emvCoreManager.EmvSetAid(aid)

        aid = addAid("A000000333010102", "0020")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0010000000")
        aid.TACOnline = PosUtils.hexStringToBytes("d84004f800")
        aid.TACDefault = PosUtils.hexStringToBytes("084000a800")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, HexUtil.parseHex("9F66046C004000"), null, null
        )
        emvCoreManager.EmvSetAid(aid)

        aid = addAid("A000000333010103", "0020")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0010000000")
        aid.TACOnline = PosUtils.hexStringToBytes("d84004f800")
        aid.TACDefault = PosUtils.hexStringToBytes("084000a800")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)

        aid = addAid("A000000333010106", "0020")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0010000000")
        aid.TACOnline = PosUtils.hexStringToBytes("d84004f800")
        aid.TACDefault = PosUtils.hexStringToBytes("084000a800")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)


        // MasterCard
        aid = addAid("A00000000410", "0002")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0400000000")
        aid.TACOnline = PosUtils.hexStringToBytes("f850acf800")
        aid.TACDefault = PosUtils.hexStringToBytes("fc50aca000")
        aid.TerminalRiskManagementData = PosUtils.hexStringToBytes("6C00000000000000")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)

        aid = addAid("A00000000430", "0002")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0400000000")
        aid.TACOnline = PosUtils.hexStringToBytes("f850acf800")
        aid.TACDefault = PosUtils.hexStringToBytes("fc50aca000")
        aid.TerminalRiskManagementData = PosUtils.hexStringToBytes("4C00800000000000")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)

        aid = addAid("A0000000043060", "0002")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0000800000")
        aid.TACOnline = PosUtils.hexStringToBytes("fc50bcf800")
        aid.TACDefault = PosUtils.hexStringToBytes("fc50bca000")
        aid.TerminalRiskManagementData = PosUtils.hexStringToBytes("4C00800000000000")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)

        aid = addAid("A0000000041010", "0002")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0000000000")
        aid.TACOnline = PosUtils.hexStringToBytes("fc50808800")
        aid.TACDefault = PosUtils.hexStringToBytes("fc50b8a000")
        aid.TerminalRiskManagementData = PosUtils.hexStringToBytes("6C00000000000000")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)


        // Discover
        aid = addAid("A0000001523010", "0001")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0010000000")
        aid.TACOnline = PosUtils.hexStringToBytes("fce09cf800")
        aid.TACDefault = PosUtils.hexStringToBytes("dc00002000")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)

        aid = addAid("A0000001524010", "0001")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0010000000")
        aid.TACOnline = PosUtils.hexStringToBytes("fce09cf800")
        aid.TACDefault = PosUtils.hexStringToBytes("dc00002000")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)

        // AMEX
        aid = addAid("A00000002501", "0001")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0000000000")
        aid.TACOnline = PosUtils.hexStringToBytes("c800000000")
        aid.TACDefault = PosUtils.hexStringToBytes("c800000000")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)



        // RuPay
        aid = addAid("A0000005241010", "0064")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)


        // MIR
        aid = addAid("A0000006581010", "0100")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0010000000")
        aid.TACOnline = PosUtils.hexStringToBytes("fc60acf800")
        aid.TACDefault = PosUtils.hexStringToBytes("fc60242800")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)

        aid = addAid("A0000006581099", "0100")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0010000000")
        aid.TACOnline = PosUtils.hexStringToBytes("fc60acf800")
        aid.TACDefault = PosUtils.hexStringToBytes("fc60242800")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)

        aid = addAid("A0000006582010", "0100")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0010000000")
        aid.TACOnline = PosUtils.hexStringToBytes("fc60acf800")
        aid.TACDefault = PosUtils.hexStringToBytes("fc60242800")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)


        // JCB
        aid = addAid("A0000000651010", "0021")
        aid.dDOL = PosUtils.hexStringToBytes("9F3704")
        aid.tDOL = PosUtils.hexStringToBytes("9F3704")
        aid.TACDenial = PosUtils.hexStringToBytes("0010000000")
        aid.TACOnline = PosUtils.hexStringToBytes("fc60acf800")
        aid.TACDefault = PosUtils.hexStringToBytes("fc60242800")
        aid.CombinationData = getKernel(
            getLimit(
                999999999999, 30000, 0, 999999999999
            ), null, null, null, null
        )
        emvCoreManager.EmvSetAid(aid)

        // Fetch all configured AID
        val configuredAidList = emvCoreManager.EmvGetAid()
        for (configuredAid in configuredAidList) {
            Log.d(
                tag,
                "AID: ${configuredAid.AID} Floor Limit : ${configuredAid.FloorLimit} ContactlessCVMLimit : ${configuredAid.ContactlessCVMLimit} ContactlessFloorLimit : ${configuredAid.ContactlessFloorLimit}"
            )
        }


    }


    fun loadCapk() {
        val emvCoreManager = POIEmvCoreManager.getDefault()
        emvCoreManager.EmvDeleteCapk()

        var capk = addCapk(
            "A000000003",
            "01",
            "",
            "03",
            "D34A6A776011C7E7CE3AEC5F03AD2F8CFC5503CC"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000003",
            "07",
            "",
            "03",
            "B4BC56CC4E88324932CBC643D6898F6FE593B172"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000003",
            "08",
            "",
            "03",
            "20D213126955DE205ADC2FD2822BD22DE21CF9A8"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000003",
            "09",
            "",
            "03",
            "1FF80A40173F52D7D27E0F26A146A1C8CCB29046"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000003",
            "57",
            "942B7F2BA5EA307312B63DF77C5243618ACC2002BD7ECB74D821FE7BDC78BF28F49F74190AD9B23B9713B140FFEC1FB429D93F56BDC7ADE4AC075D75532C1E590B21874C7952F29B8C0F0C1CE3AEEDC8DA25343123E71DCF86C6998E15F756E3",
            "010001",
            "251A5F5DE61CF28B5C6E2B5807C0644A01D46FF5"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000003",
            "92",
            "",
            "03",
            "429C954A3859CEF91295F663C963E582ED6EB253"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000003",
            "95",
            "",
            "03",
            "EE1511CEC71020A9B90443B37B1D5F6E703030F6"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000003",
            "94",
            "",
            "03",
            "C4A3C43CCF87327D136B804160E47D43B60E6E0F"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000003",
            "99",
            "",
            "03",
            "4ABFFD6B1C51212D05552E431C5B17007D2F5E6D"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000333",
            "02",
            "",
            "03",
            "03BB335A8549A03B87AB089D006F60852E4B8060"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000333",
            "03",
            "",
            "03",
            "87F0CD7C0E86F38F89A66F8C47071A8B88586F26"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000333",
            "04",
            "",
            "03",
            "F527081CF371DD7E1FD4FA414A665036E0F5E6E5"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000333",
            "08",
            "",
            "03",
            "EE23B616C95C02652AD18860E48787C079E8E85A"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000333",
            "09",
            "",
            "03",
            "A075306EAB0045BAF72CDD33B3B678779DE1F527"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000333",
            "0B",
            "",
            "03",
            "BD331F9996A490B33C13441066A09AD3FEB5F66C"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000333",
            "80",
            "",
            "010001",
            "A5E44BB0E1FA4F96A11709186670D0835057D35E"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "00",
            "",
            "03",
            "EC0A59D35D19F031E9E8CBEC56DB80E22B1DE130"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "02",
            "",
            "03",
            "33408B96C814742AD73536C72F0926E4471E8E47"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "03",
            "",
            "03",
            "5ADDF21D09278661141179CBEFF272EA384B13BB"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "04",
            "",
            "03",
            "381A035DA58B482EE2AF75F4C3F2CA469BA4AA6C"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "05",
            "",
            "03",
            "EBFA0D5D06D8CE702DA3EAE890701D45E274C845"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "06",
            "",
            "03",
            "F910A1504D5FFB793D94F3B500765E1ABCAD72D9"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "EF",
            "",
            "03",
            "21766EBB0EE122AFB65D7845B73DB46BAB65427A"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "F1",
            "",
            "03",
            "D8E68DA167AB5A85D8C3D55ECB9B0517A1A5B4BB"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "F3",
            "",
            "03",
            "A69AC7603DAF566E972DEDC2CB433E07E8B01A9A"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "F4",
            "",
            "03",
            "BF6B5B9C47134E494571732A4903C935874682B9"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "F5",
            "",
            "010001",
            "C2239804C8098170BE52D6D5D4159E81CE8466BF"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "F6",
            "",
            "03",
            "502909ED545E3C8DBD00EA582D0617FEE9F6F684"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "F7",
            "",
            "010001",
            "EEB0DD9B2477BEE3209A914CDBA94C1C4A9BDED9"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "F8",
            "",
            "03",
            "F06ECC6D2AAEBF259B7E755A38D9A9B24E2FF3DD"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "F9",
            "",
            "03",
            "336712DCC28554809C6AA9B02358DE6F755164DB"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "FA",
            "",
            "03",
            "5BED4068D96EA16D2D77E03D6036FC7A160EA99C"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000004",
            "FE",
            "",
            "03",
            "9A295B05FB390EF7923F57618A9FDA2941FC34E0"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000152",
            "5A",
            "",
            "03",
            "CC9585E8E637191C10FCECB32B5AE1B9D410B52D"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000152",
            "5B",
            "",
            "03",
            "4DC5C6CAB6AE96974D9DC8B2435E21F526BC7A60"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000152",
            "5C",
            "",
            "03",
            "60154098CBBA350F5F486CA31083D1FC474E31F8"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000152",
            "5D",
            "",
            "03",
            "B51EC5F7DE9BB6D8BCE8FB5F69BA57A04221F39B"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "03",
            "",
            "03",
            "8708A3E3BBC1BB0BE73EBD8D19D4E5D20166BF6C"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "04",
            "D0F543F03F2517133EF2BA4A1104486758630DCFE3A883C77B4E4844E39A9BD6360D23E6644E1E071F196DDF2E4A68B4A3D93D14268D7240F6A14F0D714C17827D279D192E88931AF7300727AE9DA80A3F0E366AEBA61778171737989E1EE309",
            "03",
            "FDD7139EC7E0C33167FD61AD3CADBD68D66E91C5"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "0E",
            "",
            "03",
            "A7266ABAE64B42A3668851191D49856E17F8FBCD"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "0F",
            "",
            "03",
            "A73472B3AB557493A9BC2179CC8014053B12BAB4"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "10",
            "",
            "03",
            "C729CF2FD262394ABC4CC173506502446AA9B9FD"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "65",
            "",
            "03",
            "894C5D08D4EA28BB79DC46CEAD998B877322F416"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "97",
            "",
            "03",
            "EBDA522B631B3EB4F4CBFC0679C450139D2B69CD"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "98",
            "",
            "03",
            "D4DBA428CF11D45BAEB0A35CAEA8007AD8BA8D71"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "99",
            "",
            "03",
            "94790D020F4F692D59289F36451872078005B63B"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "C1",
            "",
            "03",
            "DC79D6B5FC879362299BC5A637DAD2E0D99656B8"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "C2",
            "",
            "03",
            "8E748296359A7428F536ADDA8E2C037E2B697EF6"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "C3",
            "",
            "03",
            "12F1790CB0273DC73C6E70784BC24C12E8DB71F6"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "C7",
            "",
            "03",
            "6221E0C726BAC8F8AC25F8F93B811D1FFD4C131C"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "C8",
            "",
            "03",
            "33BD7A059FAB094939B90A8F35845C9DC779BD50"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "C9",
            "",
            "03",
            "8E8DFF443D78CD91DE88821D70C98F0638E51E49"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000025",
            "CA",
            "",
            "03",
            "6BDA32B1AA171444C7E8F88075A74FBFE845765F"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000065",
            "08",
            "",
            "03",
            "DD36D5896228C8C4900742F107E2F91FE50BC7EE"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000065",
            "09",
            "",
            "03",
            "4410C6D51C2F83ADFD92528FA6E38A32DF048D0A"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000065",
            "0F",
            "",
            "03",
            "2A1B82DE00F5F0C401760ADF528228D3EDE0F403"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000065",
            "10",
            "",
            "03",
            "C75E5210CBE6E8F0594A0F1911B07418CADB5BAB"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000065",
            "11",
            "",
            "03",
            "D9FD62C9DD4E6DE7741E9A17FB1FF2C5DB948BCB"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000065",
            "12",
            "",
            "03",
            "874B379B7F607DC1CAF87A19E400B6A9E25163E8"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000065",
            "13",
            "",
            "03",
            "54CFAE617150DFA09D3F901C9123524523EBEDF3"
        )
        emvCoreManager.EmvSetCapk(capk)
        capk = addCapk(
            "A000000065",
            "14",
            "",
            "03",
            "C0D15F6CD957E491DB56DCDD1CA87A03EBE06B7B"
        )

        emvCoreManager.EmvSetCapk(capk)


        // Fetch all configured CAPK
        val configuredCapkList = emvCoreManager.EmvGetCapk()
        for (configuredCapk in configuredCapkList) {
            Log.d(
                tag,
                "RID ${configuredCapk.RID} CAPK Index ${configuredCapk.CapkIndex} Exponent ${configuredCapk.Exponent}"
            )
        }
    }

    private fun addAid(aid: String, version: String): PosEmvAid {
        val appList = PosEmvAid()
        appList.AID = PosUtils.hexStringToBytes(aid)
        appList.Version = PosUtils.hexStringToBytes(version)
        appList.SelectIndicator = false
        appList.dDOL = PosUtils.hexStringToBytes("9F0206")
        appList.tDOL = PosUtils.hexStringToBytes("9F3704")
        appList.TACDenial = PosUtils.hexStringToBytes("0010000000")
        appList.TACOnline = PosUtils.hexStringToBytes("dc4004f800")
        appList.TACDefault = PosUtils.hexStringToBytes("dc4000a800")
        appList.Threshold = 10000
        appList.TargetPercentage = 0
        appList.MaxTargetPercentage = 99
        appList.FloorLimit = 0
        appList.DynamicTransLimit = 200000
        return appList
    }

    private fun getLimit(
        contactlessLimit: Long,
        contactlessCVMLimit: Long,
        contactlessFloorLimit: Long,
        contactlessDynamicLimit: Long,
    ): ByteArray? {
        val tlvBuilder = BerTlvBuilder()
        tlvBuilder.addBytes(BerTag("DF01"), getAmount(contactlessLimit))
        tlvBuilder.addBytes(BerTag("DF02"), getAmount(contactlessCVMLimit))
        tlvBuilder.addBytes(BerTag("DF03"), getAmount(contactlessFloorLimit))
        tlvBuilder.addBytes(BerTag("DF04"), getAmount(contactlessDynamicLimit))
        return tlvBuilder.buildArray()
    }

    private fun getAmount(value: Long): ByteArray? {
        val builder = StringBuilder(12)
        builder.append(value)
        while (builder.length < 12) {
            builder.insert(0, '0')
        }
        return HexUtil.parseHex(builder.toString())
    }

    private fun getKernel(
        kernel: ByteArray?,
        visa: ByteArray?,
        unionpay: ByteArray?,
        mastercard: ByteArray?,
        discover: ByteArray?
    ): ByteArray {
        val tlvBuilder = BerTlvBuilder()
        if (kernel != null) {
            tlvBuilder.addBytes(BerTag("DF10"), kernel)
        }
        if (visa != null) {
            tlvBuilder.addBytes(BerTag("DF11"), visa)
        }
        if (unionpay != null) {
            tlvBuilder.addBytes(BerTag("DF12"), unionpay)
        }
        if (mastercard != null) {
            tlvBuilder.addBytes(BerTag("DF13"), mastercard)
        }
        if (discover != null) {
            tlvBuilder.addBytes(BerTag("DF14"), discover)
        }
        return tlvBuilder.buildArray()
    }

    private fun addCapk(
        rid: String, capkIndex: String, modules: String, exponents: String, checksum: String
    ): PosEmvCapk {
        val capk = PosEmvCapk().apply {
            RID = PosUtils.hexStringToBytes(rid)
            CapkIndex = PosUtils.hexStringToBytes(capkIndex)[0]
            Module = PosUtils.hexStringToBytes(modules)
            Exponent = PosUtils.hexStringToBytes(exponents)
            Checksum = PosUtils.hexStringToBytes(checksum)
            AlgorithmInd = PosEmvCapk.ALGO_IND_RSA.toByte()
            HashInd = PosEmvCapk.HASH_IND_SHA1.toByte()
        }
        return capk
    }

    private fun loadVisa() {
        val bundle = Bundle()
        val tlvBuilder = BerTlvBuilder()
        val qvsdcParameter = QvsdcParameter()
        val entryPoint = EntryPoint().apply {
            CTLCheck = true
            CVMCheck = true
            CFLCheck = true
            StatusCheck = true
            ZeroAmountCheck = true
        }

        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_VISA_SET_ENTRY_POINT),
                entryPointToData(entryPoint)
            )
        )
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_VISA_SET_STATUS_ZERO_AMOUNT),
                HexUtil.parseHex("01")
            )
        )
        val kernelConfig = ByteArray(1)
        if (qvsdcParameter.SupportDRL) {
            kernelConfig[0] = (kernelConfig[0].toInt() or 0x01).toByte()
        }
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_VISA_SET_KERNEL_CONFIG), kernelConfig
            )
        )
        val qualifiers = ByteArray(4)

        // 9F66 for visa
        qualifiers[0] = 0x26.toByte()
        qualifiers[1] = 0xE0.toByte()
        qualifiers[2] = 0x40.toByte()

        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_VISA_SET_QUALIFIERS), qualifiers
            )
        )
        bundle.putByteArray(EmvTerminalConstraints.CONFIG, tlvBuilder.buildArray())
        POIEmvCoreManager.getDefault().EmvSetTerminal(EmvTerminalConstraints.TYPE_VISA, bundle)
    }

    private fun loadUnionPay() {
        val bundle = Bundle()
        val tlvBuilder = BerTlvBuilder()
        val entryPoint = EntryPoint()
        entryPoint.CTLCheck = true
        entryPoint.CVMCheck = true
        entryPoint.CFLCheck = true
        entryPoint.StatusCheck = true
        entryPoint.ZeroAmountCheck = true
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(
                    EmvTerminalConstraints.TAG_UNIONPAY_SET_ENTRY_POINT
                ), entryPointToData(entryPoint)
            )
        )
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_UNIONPAY_SET_STATUS_ZERO_AMOUNT),
                HexUtil.parseHex("01")
            )
        )
        val qualifiers = ByteArray(4)
        // 9F66 for UPI
        qualifiers[0] = 0x26.toByte()
        qualifiers[1] = 0x80.toByte()
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_UNIONPAY_SET_QUALIFIERS), qualifiers
            )
        )
        bundle.putByteArray(EmvTerminalConstraints.CONFIG, tlvBuilder.buildArray())
        POIEmvCoreManager.getDefault().EmvSetTerminal(EmvTerminalConstraints.TYPE_UNIONPAY, bundle)
    }

    private fun loadMasterCard() {
        val bundle = Bundle()
        val tlvBuilder = BerTlvBuilder()
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_MASTERCARD_SET_CVM_CAPABILITIES),
                HexUtil.parseHex("40")
            )
        )
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_MASTERCARD_SET_NO_CVM_CAPABILITIES),
                HexUtil.parseHex("08")
            )
        )
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(POIEmvCoreManager.EmvTerminalConstraints.TAG_MASTERCARD_SET_MAGSTRIPE_CVM_CAPABILITIES),
                HexUtil.parseHex("F0")
            )
        )
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(POIEmvCoreManager.EmvTerminalConstraints.TAG_MASTERCARD_SET_MAGSTRIPE_NO_CVM_CAPABILITIES),
                HexUtil.parseHex("F0")
            )
        )
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(POIEmvCoreManager.EmvTerminalConstraints.TAG_MASTERCARD_SET_DEFAULT_UDOL),
                HexUtil.parseHex("9F6A04")
            )
        )
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(POIEmvCoreManager.EmvTerminalConstraints.TAG_MASTERCARD_SET_KERNEL_CONFIG),
                HexUtil.parseHex("B0")
            )
        )
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(POIEmvCoreManager.EmvTerminalConstraints.TAG_MASTERCARD_SET_KERNEL_ID),
                HexUtil.parseHex("02")
            )
        )
        bundle.putByteArray(
            POIEmvCoreManager.EmvTerminalConstraints.CONFIG,
            tlvBuilder.buildArray()
        )

        // Configure below values
        //TAG_MASTERCARD_SET_MAGSTRIPE_APP_VERSION
        //TAG_MASTERCARD_SET_MAGSTRIPE_APP_VERSION = "9F6D"
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(POIEmvCoreManager.EmvTerminalConstraints.TAG_MASTERCARD_SET_MAGSTRIPE_APP_VERSION),
                HexUtil.parseHex("0001")
            )
        )


        POIEmvCoreManager.getDefault()
            .EmvSetTerminal(POIEmvCoreManager.EmvTerminalConstraints.TYPE_MASTERCARD, bundle)
    }

    private fun loadDiscover() {
        val bundle = Bundle()
        val tlvBuilder = BerTlvBuilder()
        val discoverParameter = DiscoverParameter()
        val entryPoint = EntryPoint()
        entryPoint.CTLCheck = true
        entryPoint.CVMCheck = true
        entryPoint.CFLCheck = true
        entryPoint.StatusCheck = true
        entryPoint.ZeroAmountCheck = true
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_DISCOVER_SET_ENTRY_POINT),
                entryPointToData(entryPoint)
            )
        )
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_DISCOVER_SET_STATUS_ZERO_AMOUNT),
                HexUtil.parseHex("01")
            )
        )
        val qualifiers = ByteArray(4)
        if (discoverParameter.SupportMagstripe) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x80).toByte()
        }
        if (discoverParameter.SupportEMV) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x20).toByte()
        }
        if (discoverParameter.SupportContact) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x10).toByte()
        }
        if (discoverParameter.OfflineOnly) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x08).toByte()
        }
        if (discoverParameter.SupportOnlinePIN) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x04).toByte()
        }
        if (discoverParameter.SupportSignature) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x02).toByte()
        }
        if (discoverParameter.SupportIssuerScriptUpdate) {
            qualifiers[2] = (qualifiers[2].toInt() or 0x80).toByte()
        }
        if (discoverParameter.SupportCDCVM) {
            qualifiers[2] = (qualifiers[2].toInt() or 0x40).toByte()
        }
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_DISCOVER_SET_QUALIFIERS), qualifiers
            )
        )
        bundle.putByteArray(EmvTerminalConstraints.CONFIG, tlvBuilder.buildArray())
        POIEmvCoreManager.getDefault().EmvSetTerminal(EmvTerminalConstraints.TYPE_DISCOVER, bundle)
    }

    fun loadAmex() {
        val bundle = Bundle()
        val tlvBuilder = BerTlvBuilder()
        val amexParameter = AmexParameter()
        val entryPoint = EntryPoint().apply {
            CTLCheck = true
            CVMCheck = true
            CFLCheck = true
            StatusCheck = true
            ZeroAmountCheck = true
        }

        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_AMEX_SET_ENTRY_POINT),
                entryPointToData(entryPoint)
            )
        )
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_AMEX_SET_STATUS_ZERO_AMOUNT),
                HexUtil.parseHex("01")
            )
        )
        val kernelConfig = ByteArray(1)
        if (amexParameter.SupportDRL) {
            kernelConfig[0] = (kernelConfig[0].toInt() or 0x01).toByte()
        }
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_AMEX_SET_KERNEL_CONFIG), kernelConfig
            )
        )
        val qualifiers = ByteArray(4)
        if (amexParameter.SupportContact) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x80).toByte()
        }
        if (amexParameter.SupportMagstripe) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x40).toByte()
        }
        if (amexParameter.TryAnotherInterface) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x04).toByte()
        }
        if (amexParameter.SupportCDCVM) {
            qualifiers[1] = (qualifiers[1].toInt() or 0x80).toByte()
        }
        if (amexParameter.SupportOnlinePIN) {
            qualifiers[1] = (qualifiers[1].toInt() or 0x40).toByte()
        }
        if (amexParameter.SupportSignature) {
            qualifiers[1] = (qualifiers[1].toInt() or 0x20).toByte()
        }
        if (amexParameter.OfflineOnly) {
            qualifiers[2] = (qualifiers[2].toInt() or 0x80).toByte()
        }
        if (amexParameter.ExemptNoCVMCheck) {
            qualifiers[3] = (qualifiers[3].toInt() or 0x80).toByte()
        }
        if (amexParameter.DelayedAuthorization) {
            qualifiers[3] = (qualifiers[3].toInt() or 0x40).toByte()
        }
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_AMEX_SET_QUALIFIERS), qualifiers
            )
        )
        bundle.putByteArray(EmvTerminalConstraints.CONFIG, tlvBuilder.buildArray())
        POIEmvCoreManager.getDefault().EmvSetTerminal(EmvTerminalConstraints.TYPE_AMEX, bundle)
    }

    fun loadMir() {
        val bundle = Bundle()
        val tlvBuilder = BerTlvBuilder()
        val mirParameter = MirParameter()
        val entryPoint = EntryPoint().apply {
            CTLCheck = true
            CVMCheck = true
            CFLCheck = true
            StatusCheck = true
            ZeroAmountCheck = true
        }

        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_MIR_SET_ENTRY_POINT), entryPointToData(entryPoint)
            )
        )
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_MIR_SET_STATUS_ZERO_AMOUNT),
                HexUtil.parseHex("01")
            )
        )
        val qualifiers = ByteArray(4)
        if (mirParameter.SupportOnlinePIN) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x80).toByte()
        }
        if (mirParameter.SupportSignature) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x40).toByte()
        }
        if (mirParameter.SupportCDCVM) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x20).toByte()
        }
        if (mirParameter.UnableOnline) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x10).toByte()
        }
        if (mirParameter.SupportContact) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x08).toByte()
        }
        if (mirParameter.OfflineOnly) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x04).toByte()
        }
        if (mirParameter.DelayedAuthorization) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x02).toByte()
        }
        if (mirParameter.ATM) {
            qualifiers[0] = (qualifiers[0].toInt() or 0x01).toByte()
        }
        tlvBuilder.addBerTlv(
            BerTlv(
                BerTag(EmvTerminalConstraints.TAG_MIR_SET_QUALIFIERS), qualifiers
            )
        )
        bundle.putByteArray(EmvTerminalConstraints.CONFIG, tlvBuilder.buildArray())
        POIEmvCoreManager.getDefault().EmvSetTerminal(EmvTerminalConstraints.TYPE_MIR, bundle)
    }

    private fun entryPointToData(entryPoint: EntryPoint): ByteArray {
        val data = ByteArray(1)
        if (entryPoint.CVMCheck) {
            data[0] = (data[0].toInt() or 0x08).toByte()
        }
        if (entryPoint.CFLCheck) {
            data[0] = (data[0].toInt() or 0x10).toByte()
        }
        if (entryPoint.CTLCheck) {
            data[0] = (data[0].toInt() or 0x20).toByte()
        }
        if (entryPoint.StatusCheck) {
            data[0] = (data[0].toInt() or 0x80).toByte()
        }
        if (entryPoint.ZeroAmountCheck) {
            data[0] = (data[0].toInt() or 0x40).toByte()
        }
        return data
    }


    fun onKernelConfig(): LiveData<Boolean> {
        return isKernelConfigCompleted
    }


}